...
MPI_Comm_size(MPI_COMM_WORLD, &size);
  ...
  int *b = new int[Ncols];
  int * data=0;
  int rowsStart= (Nrows/size) * rank;
  int rowsStop=(Nrows/size) * (rank +1);
...
MPI_Bcast(&x, 1, MPI_INT, 0, MPI_COMM_WORLD);
  ...
  for (int j = rowsStart; j < rowsStop; j++) {
      b[j] = 0;
      for (int i = 0; i < Ncols; i++) {
          A[j*Ncols+i] = 1;
      } 
  ...
  int val = (Ncols/rank); 
MPI_Gather(&b, val, MPI_INT, recv_data, 1, MPI_INT, 0, MPI_COMM_WORLD);
